...
import { calculateServiceSeoScore, serviceFormSchema, serviceAvailabilitySchema, serviceListFiltersSchema } from "@/modules/services/validation";
...
describe("Services module contract", () => {
  ...
  it("exposes a real admin resource contract compatible with the module", () => {
...
describe("Services workflow", () => {
  it("accepts every legal transition and rejects illegal ones", () => {
    ...
    for (const [from, to] of [["DRAFT", "DRAFT"], ["PUBLISHED", "PUBLISHED"], ["ARCHIVED", "PUBLISHED"], ["DELETED", "PUBLISHED"]] as const) expect(canTransitionService(from, to)).toBe(false);
...
describe("Services validation", () => {
  ...
  it("accepts valid localized service input and defaults to draft", () => {
  ...
  it("never accepts publication state through create form", () => {
    expect(serviceFormSchema.safeParse({ ...valid, status: "PUBLISHED", publishedAt: new Date() }).success).toBe(false);
  });
  ...
  it("validates admin list filters and sorting", () => {
    const parsed = serviceListFiltersSchema.parse({ page: 2, limit: 50, status: "PUBLISHED", categoryId: crypto.randomUUID(), providerId: "provider", featured: true, mobile: false, locale: "ar", sortBy: "ratingAverage100", sortOrder: "asc" });
    expect(parsed.page).toBe(2);
    expect(parsed.limit).toBe(50);
    expect(parsed.sortBy).toBe("ratingAverage100");
...
describe("Services URL contract", () => {
  it("uses organization slugs rather than database ids", () => {
    expect(buildServiceUrl("fr", null, "audit-service", "pro")).toBe("/fr/services/pro/audit-service");
    expect(buildServiceUrl("fr", "acme", "audit-service", "pro")).toBe("/fr/organizations/acme/services/pro/audit-service");
    expect(buildServiceCategoryUrl("ar", "acme", "pro")).toBe("/ar/organizations/acme/services/pro");
  });
});
