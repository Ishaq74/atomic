...
export async function getServices(input: unknown = {}, locale: Locale = "fr", publicOnly = true): Promise<{ items: ServiceListItem[]; page: number; limit: number; total: number; totalPages: number }> {
  ...
  const categories = new Map<string, { id: string; slug: string; name: string | null }[]>(); for (const category of categoryRows) categories.set(category.serviceId, [...(categories.get(category.serviceId) ?? []), { id: category.id, slug: category.slug, name: category.name ?? null }]);
  const coverMedia = new Map<string, { id: string; url: string; alt: string }>(); for (const cover of coverRows) if (cover.url) coverMedia.set(cover.serviceId, { id: cover.mediaId ?? "", url: cover.url, alt: cover.alt ?? "" });
