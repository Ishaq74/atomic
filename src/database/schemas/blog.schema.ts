...
export const blogPosts = pgTable(
  "blog_posts",
  {
    ...
    status: text("status", { enum: ["DRAFT", "PUBLISHED", "ARCHIVED", "DELETED"] })
      .default("DRAFT")
      .notNull(),
    featuredImageId: text("featured_image_id").references(() => mediaFiles.id, {
      onDelete: "set null",
    }),
    viewCount: integer("view_count").default(0).notNull(),
    isFeatured: boolean("is_featured").default(false).notNull(),
    isSticky: boolean("is_sticky").default(false).notNull(),
    commentStatus: text("comment_status", { enum: ["OPEN", "CLOSED", "DISABLED"] })
      .default("OPEN")
      .notNull(),
...
export const blogPostTranslations = pgTable(
  "blog_post_translations",
  {
    ...
    // — managed entirely by DB triggers (src/database/infra), queried via raw SQL in /api/search
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at")
      .defaultNow()
      .$onUpdate(() => /* @__PURE__ */ new Date())
...
// ─── Categories ─────────────────────────────────────────────────────────────
// Hierarchical categories scoped to an organization (or global).
...
export const blogComments = pgTable(
  "blog_comments",
  {
    ...
    content: text("content").notNull(),
    status: text("status", { enum: ["PENDING", "APPROVED", "REJECTED", "SPAM", "TRASH"] })
      .default("PENDING")
      .notNull(),
...
export const blogCommentModerations = pgTable(
  "blog_comment_moderations",
  {
    ...
    moderatorId: text("moderator_id")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }),
    action: text("action", { enum: ["APPROVE", "REJECT", "DELETE", "RESTORE", "EDIT"] })
      .notNull(),
    reason: text("reason"),
...
export const blogPostGalleryMedia = pgTable(
  "blog_post_gallery_media",
  {
    galleryId: text("gallery_id")
      .notNull()
      .references(() => blogPostGalleries.id, { onDelete: "cascade" }),
...
export const blogPostReviewHelpful = pgTable(
  "blog_post_review_helpful",
  {
    reviewId: text("review_id")
      .notNull()
      .references(() => blogPostReviews.id, { onDelete: "cascade" }),
    userId: text("user_id")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }),
...
export const blogReports = pgTable(
  "blog_reports",
  {
    ...
    reason: text("reason", { enum: ["SPAM", "ABUSIVE", "OFF_TOPIC", "HATE_SPEECH", "OTHER"] })
      .notNull(),
    description: text("description"),
...
export const blogPostReactions = pgTable(
  "blog_post_reactions",
  {
    ...
    reactionType: text("reaction_type", {
      enum: ["LIKE", "LOVE", "FIRE", "CLAP", "LAUGH", "SAD"],
    }).notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at")
      .defaultNow()
...
export const blogPostSeo = pgTable(
  "blog_post_seo",
  {
    ...
    metaTwitterCard: text("meta_twitter_card", { enum: ["summary", "summary_large_image"] }).default(
      "summary_large_image",
    ),
...
export const blogPostsRelations = relations(blogPosts, ({ one, many }) => ({
  ...
  updatedByUser: one(user, {
    ...
    references: [user.id],
    relationName: "blogPostUpdater",
  }),
  lockedByUser: one(user, {
    fields: [blogPosts.lockedBy],
    references: [user.id],
    relationName: "blogPostLocker",
  }),
  ...
  locks: one(blogPostLocks, {
    fields: [blogPosts.id],
    references: [blogPostLocks.postId],
  }),
}));
...
export const blogPostTranslationsRelations = relations(blogPostTranslations, ({ one }) => ({
  ...
  ogImage: one(mediaFiles, {
    fields: [blogPostTranslations.ogImageId],
    references: [mediaFiles.id],
  }),
}));
...
export const blogCategoriesRelations = relations(blogCategories, ({ one, many }) => ({
  ...
  parent: one(blogCategories, {
    ...
    relationName: "blogCategoryParent",
  }),
...
export const blogCommentsRelations = relations(blogComments, ({ one, many }) => ({
  ...
  parent: one(blogComments, {
    ...
    relationName: "blogCommentParent",
  }),
...
export const blogCommentModerationsRelations = relations(blogCommentModerations, ({ one }) => ({
  comment: one(blogComments, {
    fields: [blogCommentModerations.commentId],
    references: [blogComments.id],
  }),
  moderator: one(user, {
    fields: [blogCommentModerations.moderatorId],
    references: [user.id],
  }),
}));
...
export const blogPostGalleriesRelations = relations(blogPostGalleries, ({ one, many }) => ({
  post: one(blogPosts, {
    fields: [blogPostGalleries.postId],
    references: [blogPosts.id],
  }),
...
export const blogPostGalleryMediaRelations = relations(blogPostGalleryMedia, ({ one }) => ({
  gallery: one(blogPostGalleries, {
    fields: [blogPostGalleryMedia.galleryId],
    references: [blogPostGalleries.id],
  }),
  file: one(mediaFiles, {
    fields: [blogPostGalleryMedia.mediaId],
    references: [mediaFiles.id],
  }),
}));
...
export const blogPostReviewsRelations = relations(blogPostReviews, ({ one, many }) => ({
  post: one(blogPosts, {
    fields: [blogPostReviews.postId],
    references: [blogPosts.id],
  }),
  author: one(user, {
    fields: [blogPostReviews.authorId],
    references: [user.id],
  }),
...
export const blogPostReviewHelpfulRelations = relations(blogPostReviewHelpful, ({ one }) => ({
  review: one(blogPostReviews, {
    fields: [blogPostReviewHelpful.reviewId],
    references: [blogPostReviews.id],
  }),
  user: one(user, {
    fields: [blogPostReviewHelpful.userId],
    references: [user.id],
  }),
}));
...
export const blogReportsRelations = relations(blogReports, ({ one }) => ({
  post: one(blogPosts, {
    fields: [blogReports.postId],
    references: [blogPosts.id],
  }),
  comment: one(blogComments, {
    fields: [blogReports.commentId],
    references: [blogComments.id],
  }),
  review: one(blogPostReviews, {
    fields: [blogReports.reviewId],
    references: [blogPostReviews.id],
  }),
  reporter: one(user, {
    fields: [blogReports.reporterId],
    references: [user.id],
  }),
  resolver: one(user, {
    fields: [blogReports.resolvedBy],
    references: [user.id],
  }),
}));
...
export const blogPostFavoritesRelations = relations(blogPostFavorites, ({ one }) => ({
  post: one(blogPosts, {
    fields: [blogPostFavorites.postId],
    references: [blogPosts.id],
  }),
  user: one(user, {
    fields: [blogPostFavorites.userId],
    references: [user.id],
  }),
}));
...
export const blogPostReactionsRelations = relations(blogPostReactions, ({ one }) => ({
  post: one(blogPosts, {
    fields: [blogPostReactions.postId],
    references: [blogPosts.id],
  }),
  user: one(user, {
    fields: [blogPostReactions.userId],
    references: [user.id],
  }),
}));
...
export const blogPostViewStatsRelations = relations(blogPostViewStats, ({ one }) => ({
  post: one(blogPosts, {
    fields: [blogPostViewStats.postId],
    references: [blogPosts.id],
  }),
}));
...
export const blogNotificationsRelations = relations(blogNotifications, ({ one }) => ({
  ...
  comment: one(blogComments, {
    fields: [blogNotifications.commentId],
    references: [blogComments.id],
  }),
  review: one(blogPostReviews, {
    fields: [blogNotifications.reviewId],
    references: [blogPostReviews.id],
  }),
  fromUser: one(user, {
    fields: [blogNotifications.fromUserId],
    references: [user.id],
  }),
}));
...
export const blogPostLocksRelations = relations(blogPostLocks, ({ one }) => ({
  post: one(blogPosts, {
    fields: [blogPostLocks.postId],
    references: [blogPosts.id],
  }),
  user: one(user, {
    fields: [blogPostLocks.userId],
    references: [user.id],
  }),
}));
...
export const blogPostLinksRelations = relations(blogPostLinks, ({ one }) => ({
  sourcePost: one(blogPosts, {
    fields: [blogPostLinks.sourcePostId],
    references: [blogPosts.id],
    relationName: "blogPostSourceLinks",
  }),
  targetPost: one(blogPosts, {
    fields: [blogPostLinks.targetPostId],
    references: [blogPosts.id],
    relationName: "blogPostTargetLinks",
  }),
}));
