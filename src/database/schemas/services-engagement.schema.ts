...
export const serviceReactions = pgTable(
  ...
  (table) => [
    ...
    index("service_reactions_type_idx").on(table.serviceId, table.reactionType),
  ],
);
...
export const serviceNotifications = pgTable(
  "service_notifications",
  {
    ...
    type: text("type", { enum: ["NEW_COMMENT", "REPLY_TO_COMMENT", "NEW_REVIEW", "REVIEW_APPROVED", "REVIEW_REJECTED", "SERVICE_PUBLISHED", "SERVICE_MENTION"] }).notNull(),
  ...
  (table) => [
    ...
    check("service_notification_target_consistency", sql`NOT (${table.type} IN ('NEW_COMMENT','REPLY_TO_COMMENT') AND ${table.commentId} IS NULL)`),
    check("service_notification_review_target_consistency", sql`NOT (${table.type} IN ('NEW_REVIEW','REVIEW_APPROVED','REVIEW_REJECTED') AND ${table.reviewId} IS NULL)`),
  ],
);
