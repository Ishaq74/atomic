...
function translatedTenantScope(organizationId: string | null) {
  return organizationId === null
    ? isNull(blogPostTranslations.organizationId)
    : eq(blogPostTranslations.organizationId, organizationId);
}

export async function getBlogAdminPostData(
  organizationId: string | null,
  locale: Locale,
  filters: BlogAdminPostFilters = {},
) {
  const db = getDrizzle();
  ...
  const conditions = [
    ...
    translatedTenantScope(organizationId),
  ];
  ...
  if (filters.search?.trim()) {
    ...
    conditions.push(or(
      ilike(blogPostTranslations.title, query),
      ilike(blogPostTranslations.slug, query),
    )!);
  }
  ...
  if (filters.categoryId) {
    conditions.push(inArray(
      ...
      db.select({ postId: blogPostCategories.postId })
        ...
        .where(eq(blogPostCategories.categoryId, filters.categoryId)),
    ));
  }
  ...
  if (filters.tagId) {
    conditions.push(inArray(
      ...
      db.select({ postId: blogPostTags.postId })
        .from(blogPostTags)
        .where(eq(blogPostTags.tagId, filters.tagId)),
    ));
  }
  ...
  const categoryRows = postIds.length === 0
    ? []
    : await db
      .select({
        ...
        name: sql<string>`coalesce(${blogCategoryTranslations.name}, ${blogCategories.slug})`,
      })
      .from(blogPostCategories)
  ...
  for (const category of categoryRows) {
    ...
    else categoriesByPost.set(category.postId, [category]);
  }

  const items = rows.map((row) => ({
    ...row,
    categories: (categoriesByPost.get(row.post.id) ?? []).map((category) => ({
      id: category.id,
      slug: category.slug,
      name: category.name,
    })),
  ...
  const [{ total: aggregateTotal }] = await db
    .select({ total: count() })
    .from(blogPosts)
    .where(tenantScope(organizationId));

  const statusRows = await db
    .select({ status: blogPosts.status, count: count() })
    .from(blogPosts)
    .where(tenantScope(organizationId))
    .groupBy(blogPosts.status);

  const [{ views }] = await db
    .select({ views: sql<number>`coalesce(sum(${blogPosts.viewCount}), 0)` })
    .from(blogPosts)
    .where(tenantScope(organizationId));
  ...
  const [{ featured }] = await db
    ...
    .from(blogPosts)
    .where(and(tenantScope(organizationId), eq(blogPosts.isFeatured, true)));

  const [{ sticky }] = await db
    .select({ sticky: count() })
    .from(blogPosts)
    .where(and(tenantScope(organizationId), eq(blogPosts.isSticky, true)));
...
export async function getBlogAdminAuthors(organizationId: string | null) {
  ...
  return db
    ...
    .from(user)
    .innerJoin(blogPosts, eq(blogPosts.authorId, user.id))
    .where(tenantScope(organizationId))
...
export async function getBlogAdminTags(organizationId: string | null, locale: Locale) {
  ...
  return db
    .select({
      id: blogTags.id,
      slug: blogTags.slug,
      name: sql<string>`coalesce(${blogTagTranslations.name}, ${blogTags.slug})`,
    })
    .from(blogTags)
    .leftJoin(
      blogTagTranslations,
      and(
        eq(blogTagTranslations.tagId, blogTags.id),
        eq(blogTagTranslations.locale, locale),
      ),
    )
    .where(
      organizationId === null
        ? isNull(blogTags.organizationId)
        : eq(blogTags.organizationId, organizationId),
    )
    .orderBy(asc(sql<string>`coalesce(${blogTagTranslations.name}, ${blogTags.slug})`));
}
