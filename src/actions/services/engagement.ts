import { ActionError, and, count, desc, eq } from "drizzle-orm";
...
export const createServiceReview = defineAction({
  ...
  handler: async (input, context) => {
    ...
    const locale = requestLocale(context.request.headers); const t = getServiceTranslations(locale);
...
export const createServiceComment = defineAction({
  ...
  handler: async (input, context) => {
    ...
    const db = getDrizzle(); let recipientId = service.providerId; let type: "NEW_COMMENT" | "REPLY_TO_COMMENT" = "NEW_COMMENT"; const locale = requestLocale(context.request.headers); const t = getServiceTranslations(locale);
...
export const createServiceReport = defineAction({
  input: serviceIdInput.extend({ commentId: z.uuid().optional(), reviewId: z.uuid().optional(), reason: z.enum(["SPAM", "ABUSIVE", "OFF_TOPIC", "HATE_SPEECH", "OTHER"]), description: z.string().trim().max(2000).optional() }).refine((v) => Number(Boolean(v.commentId)) + Number(Boolean(v.reviewId)) === 1, { message: "Un signalement doit cibler exactement un commentaire ou un avis." }),
  handler: async (input, context) => {
