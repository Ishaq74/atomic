...
export const moderateServiceReview = defineAction({ input: z.object({ id: z.uuid(), serviceId: z.uuid(), organizationId: serviceOrganizationIdSchema, status: z.enum(["APPROVED", "REJECTED", "SPAM"]) }), handler: async (input, context) => {
  const tenant = resolveServiceTenant(input); const user = await assertServicePermission(context, tenant, { serviceReview: ["moderate"] }); const service = await assertServiceInTenant(input.serviceId, tenant);
  const db = getDrizzle();
  const updated = await db.update(serviceReviews).set({ status: input.status }).where(and(eq(serviceReviews.id, input.id), eq(serviceReviews.serviceId, input.serviceId))).returning({ id: serviceReviews.id, authorId: serviceReviews.authorId });
...
export const resolveServiceReport = defineAction({ input: z.object({ id: z.uuid(), serviceId: z.uuid(), organizationId: serviceOrganizationIdSchema, status: z.enum(["REVIEWED", "RESOLVED", "REJECTED"]) }), handler: async (input, context) => {
  ...
  if (report.commentId) { const [comment] = await db.select({ serviceId: serviceComments.serviceId }).from(serviceComments).where(eq(serviceComments.id, report.commentId)).limit(1); if (!comment || comment.serviceId !== input.serviceId) throw new ActionError({ code: "FORBIDDEN", message: "Ce signalement ne cible pas ce service." }); }
  if (report.reviewId) { const [review] = await db.select({ serviceId: serviceReviews.serviceId }).from(serviceReviews).where(eq(serviceReviews.id, report.reviewId)).limit(1); if (!review || review.serviceId !== input.serviceId) throw new ActionError({ code: "FORBIDDEN", message: "Ce signalement ne cible pas ce service." }); }
