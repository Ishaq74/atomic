...
import { assertServicePermission, resolveServiceTenant, serviceOrganizationIdSchema } from "@/modules/services/permissions";
...
const tenantPredicate = (organizationId: string | null) => organizationId === null ? isNull(services.organizationId) : eq(services.organizationId, organizationId);
const targetStatus: Record<BulkOperation, ServiceStatus> = { publish: "PUBLISHED", archive: "ARCHIVED", restore: "DRAFT", delete: "DELETED" };
...
const auditFor: Record<BulkOperation, "SERVICE_PUBLISH" | "SERVICE_ARCHIVE" | "SERVICE_RESTORE" | "SERVICE_DELETE"> = { publish: "SERVICE_PUBLISH", archive: "SERVICE_ARCHIVE", restore: "SERVICE_RESTORE", delete: "SERVICE_DELETE" };
...
export const bulkServiceLifecycle = defineAction({
  ...
  handler: async (input, context) => {
    ...
    const db = getDrizzle();
    const rows = await db.select().from(services).where(and(tenantPredicate(tenant.organizationId), inArray(services.id, input.ids)));
    ...
    for (const lock of locks) {
      if (lock.expiresAt > now && (lock.userId !== user.id || lock.sessionId !== sessionId)) throw new ActionError({ code: "CONFLICT", message: "Un des services sélectionnés est actuellement verrouillé par un autre éditeur." });
    }
    ...
    await db.transaction(async (tx) => {
      for (const row of rows) {
        ...
        if (translation) await tx.insert(serviceRevisions).values({ serviceId: row.id, authorId: user.id, locale: translation.locale, title: translation.title, slug: translation.slug, content: translation.content, excerpt: translation.excerpt, status: to === "PUBLISHED" ? "PUBLISHED" : to === "ARCHIVED" ? "ARCHIVED" : "DRAFT", revisionNote: `Action groupée : ${input.operation}` });
      }
    });
    for (const row of rows) auditService(context, user.id, auditFor[input.operation], { resource: "services", resourceId: row.id, metadata: { organizationId: tenant.organizationId, bulk: true, from: row.status, to } });
