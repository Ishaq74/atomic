...
export const listServiceNotifications = defineAction({ input: notificationInput.extend({ limit: z.coerce.number().int().min(1).max(100).default(50) }), handler: async (input, context) => {
...
export const markServiceNotificationRead = defineAction({ input: z.object({ id: z.uuid(), organizationId: serviceOrganizationIdSchema }), handler: async (input, context) => {
  ...
  if (!notification) return { success: false }; await db.update(serviceNotifications).set({ readAt: new Date() }).where(eq(serviceNotifications.id, notification.id)); return { success: true };
} });

export const markAllServiceNotificationsRead = defineAction({ input: notificationInput, handler: async (input, context) => {
  const tenant = resolveServiceTenant(input); const user = await assertServicePermission(context, tenant, { service: ["read"] }); const db = getDrizzle();
  ...
  if (rows.length) await db.update(serviceNotifications).set({ readAt: new Date() }).where(inArray(serviceNotifications.id, rows.map((row) => row.id))); return { success: true };
} });
