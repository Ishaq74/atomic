...
const attributeDefinitionInput = z.object({
  ...
  if (value.type !== "SELECT" && value.options.length) ctx.addIssue({ code: "custom", path: ["options"], message: "Options are only valid for SELECT attributes." });
...
export const createServiceAttributeDefinition = defineAction({
  ...
  handler: async (input, context) => {
    ...
    const db = getDrizzle();
    const existing = await db.select({ id: serviceAttributeDefinitions.id }).from(serviceAttributeDefinitions).where(and(eq(serviceAttributeDefinitions.key, input.key), tenant.organizationId === null ? eq(serviceAttributeDefinitions.organizationId, null) : eq(serviceAttributeDefinitions.organizationId, tenant.organizationId))).limit(1);
    ...
    auditService(context, user.id, "SERVICE_UPDATE", { resource: "serviceAttributeDefinitions", resourceId: row.id, metadata: { organizationId: tenant.organizationId, action: "CREATE" } });
...
export const setServiceAttributeValue = defineAction({
  ...
  handler: async (input, context) => {
    ...
    const definition = await assertAttributeDefinitionInTenant(input.definitionId, tenant);
    const supplied = [input.stringValue !== null && input.stringValue !== undefined, input.numberValue !== null && input.numberValue !== undefined, input.booleanValue !== null && input.booleanValue !== undefined, input.selectedValue !== null && input.selectedValue !== undefined].filter(Boolean).length;
    if (supplied > 1) throw new ActionError({ code: "BAD_REQUEST", message: "Une seule représentation de valeur peut être fournie." });
    if (definition.type === "STRING" && input.stringValue === undefined && input.stringValue !== null) throw new ActionError({ code: "BAD_REQUEST", message: "Une valeur texte est requise." });
    if (definition.type === "NUMBER" && input.numberValue === undefined && input.numberValue !== null) throw new ActionError({ code: "BAD_REQUEST", message: "Une valeur numérique est requise." });
    if (definition.type === "BOOLEAN" && input.booleanValue === undefined && input.booleanValue !== null) throw new ActionError({ code: "BAD_REQUEST", message: "Une valeur booléenne est requise." });
    if (definition.type === "SELECT") {
      if (input.selectedValue === undefined && input.selectedValue !== null) throw new ActionError({ code: "BAD_REQUEST", message: "Une option est requise." });
      const options = definition.options ? JSON.parse(definition.options) as string[] : [];
      if (input.selectedValue && !options.includes(input.selectedValue)) throw new ActionError({ code: "BAD_REQUEST", message: "Option d'attribut invalide." });
    }
    const db = getDrizzle();
    ...
    auditService(context, user.id, "SERVICE_UPDATE", { resource: "serviceAttributeValues", resourceId: input.serviceId, metadata: { organizationId: tenant.organizationId, definitionId: input.definitionId } });
    invalidateServicesCache();
    return { success: true };
  },
});
