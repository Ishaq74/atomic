...
export const setServiceAttributeValue = defineAction({
  input: z.object({ serviceId: z.uuid(), organizationId: serviceOrganizationIdSchema, definitionId: z.uuid(), stringValue: z.string().optional().nullable(), numberValue: z.coerce.number().int().optional().nullable(), booleanValue: z.boolean().optional().nullable(), selectedValue: z.string().optional().nullable() }),
  handler: async (input, context) => {
    ...
    if (definition.type === "STRING" && input.stringValue === undefined) throw new ActionError({ code: "BAD_REQUEST", message: "Une valeur texte est requise." });
    if (definition.type === "NUMBER" && input.numberValue === undefined) throw new ActionError({ code: "BAD_REQUEST", message: "Une valeur numérique est requise." });
    if (definition.type === "BOOLEAN" && input.booleanValue === undefined) throw new ActionError({ code: "BAD_REQUEST", message: "Une valeur booléenne est requise." });
    if (definition.type === "SELECT") { if (input.selectedValue === undefined) throw new ActionError({ code: "BAD_REQUEST", message: "Une option est requise." }); const options = definition.options ? JSON.parse(definition.options) as string[] : []; if (input.selectedValue && !options.includes(input.selectedValue)) throw new ActionError({ code: "BAD_REQUEST", message: "Option d'attribut invalide." }); }
    const db = getDrizzle();
