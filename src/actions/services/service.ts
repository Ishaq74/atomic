...
export const createService = defineAction({
  ...
  handler: async (input, context) => {
    ...
    if (input.status !== "DRAFT" || input.publishedAt !== null) {
      throw new ActionError({ code: "BAD_REQUEST", message: "Un service doit être créé en brouillon. Utilisez la publication explicite ensuite." });
    }
...
export const updateService = defineAction({
  ...
  handler: async (input, context) => {
    ...
    const seoScore = calculateServiceSeoScore({
      title: input.title ?? existingTranslation?.title,
      metaTitle: input.metaTitle ?? existingTranslation?.metaTitle,
      metaDescription: input.metaDescription ?? existingTranslation?.metaDescription,
      focusKeyword: input.focusKeyword !== undefined ? input.focusKeyword : existingSeo?.focusKeyword ?? undefined,
    });
        ...
        if (locale) {
          if (existingTranslation) {
            await tx.update(serviceTranslations).set({
              ...
              ...(input.metaTitle !== undefined ? { metaTitle: input.metaTitle } : input.title !== undefined ? { metaTitle: input.title } : {}),
