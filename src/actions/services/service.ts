  ...
  handler: async (input, context) => {
    ...
    try {
      await db.transaction(async (tx) => {
        ...
        if (input.tagIds) { await tx.delete(serviceTagLinks).where(eq(serviceTagLinks.serviceId, input.id)); if (input.tagIds.length) await tx.insert(serviceTagLinks).values(input.tagIds.map((categoryId) => ({ serviceId: input.id, tagId: categoryId }))); }
