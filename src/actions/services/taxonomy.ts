...
export const updateServiceCategory = defineAction({ input: categoryInput.extend({ id: z.uuid() }), handler: async (input, context) => {
  const tenant = resolveServiceTenant(input); const user = await assertServicePermission(context, tenant, { serviceCategory: ["update"] }); await assertServiceCategoryInTenant(input.id, tenant); if (input.parentId) await assertServiceCategoryInTenant(input.parentId, tenant);
  const db = getDrizzle(); const nodes = await db.select({ id: serviceCategories.id, parentId: serviceCategories.parentId }).from(serviceCategories).where(tenant.organizationId === null ? isNull(serviceCategories.organizationId) : eq(serviceCategories.organizationId, tenant.organizationId)); assertAcyclicParent(nodes, input.id, input.parentId ?? null);
  await db.update(serviceCategories).set({ parentId: input.parentId ?? null, slug: input.slug, icon: input.icon ?? null, color: input.color ?? null, sortOrder: input.sortOrder }).where(eq(serviceCategories.id, input.id));
...
export const deleteServiceCategory = defineAction({ input: z.object({ id: z.uuid(), organizationId: serviceOrganizationIdSchema }), handler: async (input, context) => {
  const tenant = resolveServiceTenant(input); const user = await assertServicePermission(context, tenant, { serviceCategory: ["delete"] }); await assertServiceCategoryInTenant(input.id, tenant); const db = getDrizzle(); const child = await db.select({ id: serviceCategories.id }).from(serviceCategories).where(eq(serviceCategories.parentId, input.id)).limit(1); if (child.length) throw new ActionError({ code: "CONFLICT", message: "Cette catégorie possède des sous-catégories." }); await db.delete(serviceCategories).where(eq(serviceCategories.id, input.id)); auditService(context, user.id, "SERVICE_CATEGORY_DELETE", { resource: "serviceCategories", resourceId: input.id }); invalidateServicesCache(); return { success: true };
} });
