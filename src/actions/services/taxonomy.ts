...
export const updateServiceCategory = defineAction({ input: categoryInput.extend({ id: z.uuid() }), handler: async (input, context) => {
  ...
  if (!current) throw new ActionError({ code: "NOT_FOUND", message: getServiceErrorMessage(input.locale, "NOT_FOUND") });
  ...
  const baseUpdate = input.locale === DEFAULT_LOCALE ? { slug: input.slug } : {};
  await db.update(serviceCategories).set({ parentId: nextParentId ?? null, ...baseUpdate, icon: input.icon ?? null, color: input.color ?? null, sortOrder: input.sortOrder }).where(eq(serviceCategories.id, input.id));
...
export const deleteServiceCategory = defineAction({ input: z.object({ id: z.uuid(), organizationId: serviceOrganizationIdSchema, locale: z.enum(["fr", "en", "es", "ar"]).default("fr") }), handler: async (input, context) => {
  const tenant = resolveServiceTenant(input); const user = await assertServicePermission(context, tenant, { serviceCategory: ["delete"] }); await assertServiceCategoryInTenant(input.id, tenant); const db = getDrizzle(); const child = await db.select({ id: serviceCategories.id }).from(serviceCategories).where(and(eq(serviceCategories.parentId, input.id), tenant.organizationId === null ? isNull(serviceCategories.organizationId) : eq(serviceCategories.organizationId, tenant.organizationId))).limit(1); if (child.length) throw new ActionError({ code: "CONFLICT", message: getServiceErrorMessage(input.locale, "CONFLICT") }); await db.delete(serviceCategories).where(eq(serviceCategories.id, input.id)); auditService(context, user.id, "SERVICE_CATEGORY_DELETE", { resource: "serviceCategories", resourceId: input.id }); invalidateServicesCache(); return { success: true };
} });
