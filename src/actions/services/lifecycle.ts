...
export const lockService = defineAction({ input: lifecycleInput, handler: async (input, context) => {
  const locale = requestLocale(context.request.headers); const tenant = resolveServiceTenant(input); const user = await assertServicePermission(context, tenant, { service: ["update"] }); await assertServiceInTenant(input.id, tenant); const db = getDrizzle(); const existing = (await db.select().from(serviceLocks).where(eq(serviceLocks.serviceId, input.id)).limit(1))[0]; const now = new Date(); const sessionId = context.locals.session?.id;
  if (existing && existing.expiresAt > now && (existing.userId !== user.id || existing.sessionId !== (sessionId ?? ""))) throw new ActionError({ code: "CONFLICT", message: getServiceTranslations(locale).admin.errors.conflict });
  const expiresAt = new Date(Date.now() + 30 * 60 * 1000); await db.insert(serviceLocks).values({ serviceId: input.id, userId: user.id, sessionId: sessionId ?? "", expiresAt }).onConflictDoUpdate({ target: serviceLocks.serviceId, set: { userId: user.id, sessionId: sessionId ?? "", lockedAt: now, expiresAt } }); auditService(context, user.id, "SERVICE_LOCK", { resource: "services", resourceId: input.id }); return { success: true, expiresAt };
} });
