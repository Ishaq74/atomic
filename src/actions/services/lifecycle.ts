...
function tenantPredicate(column: typeof services.organizationId, organizationId: string | null) { return organizationId === null ? isNull(column) : eq(column, organizationId); }
async function uniqueServiceSlug(organizationId: string | null, sourceSlug: string): Promise<string> { const rows = await getDrizzle().select({ slug: services.slug }).from(services).where(tenantPredicate(services.organizationId, organizationId)); const used = new Set(rows.map((row) => row.slug)); const base = `${sourceSlug}-copy`; let candidate = base; let suffix = 2; while (used.has(candidate)) candidate = `${base}-${suffix++}`; return candidate; }
async function uniqueTranslationSlug(organizationId: string | null, locale: string, sourceSlug: string): Promise<string> { const rows = await getDrizzle().select({ slug: serviceTranslations.slug }).from(serviceTranslations).where(and(tenantPredicate(serviceTranslations.organizationId, organizationId), eq(serviceTranslations.locale, locale))); const used = new Set(rows.map((row) => row.slug)); const base = `${sourceSlug}-copy`; let candidate = base; let suffix = 2; while (used.has(candidate)) candidate = `${base}-${suffix++}`; return candidate; }
...
async function transitionService(id: string, organizationId: string | null | undefined, to: ServiceStatus, context: Parameters<typeof assertServicePermission>[0], permission: LifecyclePermission, auditAction: LifecycleAuditAction) {
  ...
  if (to === "PUBLISHED") { const t = getServiceNotificationTranslations(requestLocale(context.request.headers)); await createServiceNotification({ recipientId: current.providerId, serviceId: id, actorId: user.id, type: "SERVICE_PUBLISHED", title: t.publishedTitle, message: t.publishedMessage, locale: requestLocale(context.request.headers) }); }
...
export const duplicateService = defineAction({ input: lifecycleInput, handler: async (input, context) => {
  ...
  const [translations, categories, tags, media, availability, seo, attributes] = await Promise.all([
    db.select().from(serviceTranslations).where(eq(serviceTranslations.serviceId, source.id)).orderBy(serviceTranslations.locale),
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      const first = translations[0]; if (first) await tx.insert(serviceRevisions).values({ serviceId: duplicateId, authorId: user.id, locale: first.locale, title: first.title, slug: translationSlugs.get(first.locale)!, content: first.content, excerpt: first.excerpt, status: "DRAFT", revisionNote: "Duplication" });
    });
  } catch (error) {
    if (error instanceof Error && /duplicate|unique/i.test(error.message)) throw new ActionError({ code: "CONFLICT", message: getServiceTranslations(requestLocale(context.request.headers)).admin.errors.conflict });
    throw error;
  }
...
export const lockService = defineAction({ input: lifecycleInput, handler: async (input, context) => {
  const locale = requestLocale(context.request.headers); const tenant = resolveServiceTenant(input); const user = await assertServicePermission(context, tenant, { service: ["update"] }); await assertServiceInTenant(input.id, tenant); const db = getDrizzle(); const existing = (await db.select().from(serviceLocks).where(eq(serviceLocks.serviceId, input.id)).limit(1))[0]; const now = new Date();
  if (existing && existing.expiresAt > now && existing.userId !== user.id) throw new ActionError({ code: "CONFLICT", message: getServiceTranslations(locale).admin.errors.conflict });
  const expiresAt = new Date(Date.now() + 30 * 60 * 1000); await db.insert(serviceLocks).values({ serviceId: input.id, userId: user.id, sessionId: context.locals.session?.id ?? "unknown", expiresAt }).onConflictDoUpdate({ target: serviceLocks.serviceId, set: { userId: user.id, sessionId: context.locals.session?.id ?? "unknown", lockedAt: now, expiresAt } }); auditService(context, user.id, "SERVICE_LOCK", { resource: "services", resourceId: input.id }); return { success: true, expiresAt };
} });
...
export const restoreServiceRevision = defineAction({ input: z.object({ revisionId: z.uuid(), serviceId: z.uuid(), organizationId: serviceOrganizationIdSchema }), handler: async (input, context) => {
  ...
  auditService(context, user.id, "SERVICE_RESTORE", { resource: "services", resourceId: input.serviceId, metadata: { organizationId: tenant.organizationId, restoredRevisionId: input.revisionId } }); invalidateServicesCache(); return { success: true };
} });
