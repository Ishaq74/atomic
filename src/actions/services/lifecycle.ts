...
export const duplicateService = defineAction({ input: lifecycleInput, handler: async (input, context) => {
  ...
  const existingSlugs = new Set((await db.select({ slug: serviceTranslations.slug }).from(serviceTranslations).innerJoin(services, eq(services.id, serviceTranslations.serviceId)).where(and(tenant.organizationId === null ? undefined : eq(services.organizationId, tenant.organizationId)))).map((row) => row.slug));
  ...
  const localeSlugs = new Map<string, string>();
  for (const translation of translations) localeSlugs.set(translation.locale, makeUnique(`${translation.slug}-copy`));
  let duplicateId = "";
  await db.transaction(async (tx) => {
    const canonicalSlug = makeUnique(baseSlug);
    const [created] = await tx.insert(services).values({ organizationId: tenant.organizationId, providerId: source.providerId, slug: canonicalSlug, status: "DRAFT", coverImageId: source.coverImageId, priceMinor: source.priceMinor, currency: source.currency, durationMinutes: source.durationMinutes, maxParticipants: source.maxParticipants, isMobile: source.isMobile, isFeatured: false, seoScore: null, publishedAt: null, updatedBy: user.id }).returning({ id: services.id });
    ...
    if (first) await tx.insert(serviceRevisions).values({ serviceId: duplicateId, authorId: user.id, locale: first.locale, title: `${first.title} (copy)`, slug: localeSlugs.get(first.locale) ?? `${first.slug}-copy`, content: first.content, excerpt: first.excerpt, status: "DRAFT", revisionNote: "Duplication" });
  });
...
export const restoreServiceRevision = defineAction({ input: z.object({ revisionId: z.uuid(), serviceId: z.uuid(), organizationId: serviceOrganizationIdSchema }), handler: async (input, context) => {
  const tenant = resolveServiceTenant(input); const current = await assertServiceInTenant(input.serviceId, tenant); const db = getDrizzle(); const [revision] = await db.select().from(serviceRevisions).where(and(eq(serviceRevisions.id, input.revisionId), eq(serviceRevisions.serviceId, input.serviceId))).limit(1);
