...
function tenantPredicate(column: typeof services.organizationId, organizationId: string | null) {
  return organizationId === null ? isNull(column) : eq(column, organizationId);
}

async function uniqueServiceSlug(organizationId: string | null, sourceSlug: string): Promise<string> {
  const db = getDrizzle();
  const rows = await db.select({ slug: services.slug }).from(services).where(tenantPredicate(services.organizationId, organizationId));
...
async function uniqueTranslationSlug(organizationId: string | null, locale: string, sourceSlug: string): Promise<string> {
  const db = getDrizzle();
  const rows = await db.select({ slug: serviceTranslations.slug }).from(serviceTranslations).where(and(tenantPredicate(serviceTranslations.organizationId, organizationId), eq(serviceTranslations.locale, locale)));
  const used = new Set(rows.map((row) => row.slug));
  const base = `${sourceSlug}-copy`;
  let candidate = base;
  let suffix = 2;
...
export const duplicateService = defineAction({ input: lifecycleInput, handler: async (input, context) => {
  ...
  const serviceSlug = await uniqueServiceSlug(tenant.organizationId, source.slug);
  const translationSlugs = new Map<string, string>();
  for (const translation of translations) translationSlugs.set(translation.locale, await uniqueTranslationSlug(tenant.organizationId, translation.locale, translation.slug));
  let duplicateId = "";
  try {
    await db.transaction(async (tx) => {
      ...
      for (const translation of translations) {
        await tx.insert(serviceTranslations).values({ serviceId: duplicateId, organizationId: tenant.organizationId, locale: translation.locale, title: `${translation.title} (copy)`, slug: translationSlugs.get(translation.locale)!, excerpt: translation.excerpt, content: translation.content, locationLabel: translation.locationLabel, locationAddress: translation.locationAddress, metaTitle: translation.metaTitle, metaDescription: translation.metaDescription, metaKeywords: translation.metaKeywords, canonicalUrl: null, ogTitle: translation.ogTitle, ogDescription: translation.ogDescription, ogImageId: translation.ogImageId });
      }
      ...
      if (first) await tx.insert(serviceRevisions).values({ serviceId: duplicateId, authorId: user.id, locale: first.locale, title: `${first.title} (copy)`, slug: translationSlugs.get(first.locale)!, content: first.content, excerpt: first.excerpt, status: "DRAFT", revisionNote: "Duplication" });
    });
  } catch (error) {
    if (error instanceof Error && /duplicate|unique/i.test(error.message)) throw new ActionError({ code: "CONFLICT", message: "Impossible de créer une duplication avec des slugs uniques." });
    throw error;
  }
...
export const restoreServiceRevision = defineAction({ input: z.object({ revisionId: z.uuid(), serviceId: z.uuid(), organizationId: serviceOrganizationIdSchema }), handler: async (input, context) => {
  ...
  const user = await assertServicePermission(context, tenant, { service: [revision.status === "PUBLISHED" ? "publish" : "update"] });
  ...
  try {
    ...
    if (error instanceof Error && /duplicate|unique/i.test(error.message)) throw new ActionError({ code: "CONFLICT", message: "La restauration entre en conflit avec un slug déjà utilisé." });
    throw error;
  }
  if (revision.status === "PUBLISHED") { const t = getServiceNotificationTranslations(requestLocale(context.request.headers)); await createServiceNotification({ recipientId: current.providerId, serviceId: input.serviceId, actorId: user.id, type: "SERVICE_PUBLISHED", title: t.publishedTitle, message: t.publishedMessage }); }
