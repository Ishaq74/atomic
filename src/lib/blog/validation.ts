...
export function assertSlugNotReserved(slug: string, message = `Le slug « ${slug} » est réservé.`) {
  if (BLOG_RESERVED_SLUGS.has(slug)) {
...
export const blogSlugSchema = z
  ...
  .refine((s) => !BLOG_RESERVED_SLUGS.has(s), {
...
export const blogReportFormSchema = z.object({
  ...
  const targets = [data.postId, data.commentId, data.reviewId].filter(Boolean);
...
export function calculateSeoScore(input: {
  ...
  if (mt.length >= BLOG_SEO_LIMITS.titleMin && mt.length <= BLOG_SEO_LIMITS.titleMax) score += 25;
  else if (mt.length > 0) score += 10;

  if (input.metaDescription && input.metaDescription.length >= BLOG_SEO_LIMITS.descriptionMin &&
      input.metaDescription.length <= BLOG_SEO_LIMITS.descriptionMax) score += 25;

  if (input.content.length >= BLOG_SEO_LIMITS.contentMin) score += 25;
  else score += Math.round((input.content.length / BLOG_SEO_LIMITS.contentMin) * 25);

  if (input.focusKeyword) {
    const kw = input.focusKeyword.toLowerCase();
    const content = input.content.toLowerCase();
    const count = content.split(kw).length - 1;
    if (count >= 1 && count <= BLOG_SEO_LIMITS.focusKeywordMax) score += 15;
